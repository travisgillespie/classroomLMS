<?php 
function download_csvSkyward($firstName,$lastName,$assType,$title,$attempt,$gradeLevel,$period){
if (isset($_POST['downloadSkyward'])) {
	global $connection;
	$safe_firstName = mysqli_real_escape_string($connection, $firstName);
	$safe_lastName = mysqli_real_escape_string($connection, $lastName);
		$query  = "SELECT firstName, lastName, student_id, score ";
...
download_csvSkyward($firstName,$lastName,$assType,$title,$attempt,$gradeLevel,$period); 
?>
